...
public class KinectV2Manager : KinectManager
{
	...
	{
		base.minDepth = ushort.MaxValue;
		base.maxDepth = ushort.MinValue;
		base.pointCloudManager = GameObject.FindObjectOfType<KinectPointCloud>();
		base.gradedLevels = new ushort[pointCloudManager.gradeLevels.Length];
	...
	void Update()
	{
		for (int i = 0; i < pointCloudManager.gradeLevels.Length; i++)
			base.gradedLevels[i] = pointCloudManager.gradeLevels[i];
				...
				for (int y = meshY * processedKinectData.MeshHeight; y < (meshY + 1) * processedKinectData.MeshHeight; y++)
				{
					for (int x = meshX * processedKinectData.MeshWidth; x < (meshX + 1) * processedKinectData.MeshWidth; x++)
					{
						...
						if (!float.IsNegativeInfinity(colorSpacePoint.X) &&
							...
							base.processedKinectData.CorrectedDepths[depthSpaceIndex] >= base.gradedLevels[0] &&
							base.processedKinectData.CorrectedDepths[depthSpaceIndex] < base.gradedLevels[5])
						{
							...
							{
								// update mesh colors
								if(base.pointCloudManager.useShader)
								{
									if (base.processedKinectData.CorrectedDepths[depthSpaceIndex] > gradedLevels[0])
										processedKinectData.MeshColors[meshIndex][smallIndex] = new Color(0, 0, 0, 1);
									if (base.processedKinectData.CorrectedDepths[depthSpaceIndex] > gradedLevels[1])
										processedKinectData.MeshColors[meshIndex][smallIndex] = new Color(0, 1, 0, 0);
									if (base.processedKinectData.CorrectedDepths[depthSpaceIndex] > gradedLevels[2])
										processedKinectData.MeshColors[meshIndex][smallIndex] = new Color(0, 0, 1, 0);
									if (base.processedKinectData.CorrectedDepths[depthSpaceIndex] > gradedLevels[3])
										processedKinectData.MeshColors[meshIndex][smallIndex] = new Color(1, 0, 0, 0);
									if (base.processedKinectData.CorrectedDepths[depthSpaceIndex] > gradedLevels[4])
										processedKinectData.MeshColors[meshIndex][smallIndex] = new Color(0, 0, 0, 0);

									if (maxDepth < base.processedKinectData.CorrectedDepths[depthSpaceIndex])
										maxDepth = base.processedKinectData.CorrectedDepths[depthSpaceIndex];

									if (minDepth > base.processedKinectData.CorrectedDepths[depthSpaceIndex])
										minDepth = base.processedKinectData.CorrectedDepths[depthSpaceIndex];
								} else
									processedKinectData.MeshColors[meshIndex][smallIndex] = pixelColor;
							...
							if (displayStreams)
							{
								byte infraredPixelIntensity = (byte)(base.processedKinectData.RawDepths[depthSpaceIndex] >> 5);
